...
namespace MainProgram
{
    class RPS
    {
        ...
        public int ScorePartOne(string[] currentRound)
        {
        ...
        public void PartOne()
        {
            ...
            foreach (string line in lines) // This is loop through every line in tne input.txt file
            {
                ...
                int roundScore = rps.ScorePartOne(currentRound); // This invoked the Score() and return an integer score, depending on the values of the user and the enemy
        ...
        // ===== This is part 2 of the challege, this was given after I have solved part 1 =====
        
        public string ReturnShape = " ";

        public int ScorePartTwo(string[] currentRound)
        {
            // This function deals will the game
            // Like when the user has X(Rock) and the enemy draws A(Rock) it will be a draw, returning 3 as stated in the rules.
            RPS rps = new RPS();
            ...
            if(currentRound[1] == "Y")
            {
                ...
                else if(currentRound[0] == "B")
                {
                    ReturnShape = "Y";
                    return 3;        
                }
                else if(currentRound[0] == "C")
                {
                    ReturnShape = "Z";   
                    return 3;     
                }
            }
            else if(currentRound[1] == "X")
            {
                if(currentRound[0] == "A")
                {
                    ReturnShape = "Z";
        ...
        public void PartTwo()
        {
            ...
            {
                currentRound = line.Split(' '); // For every line the File it will split the string into 2, these are strings separted by space
                int roundScore = rps.ScorePartTwo(currentRound); // This invoked the Score() and return an integer score, depending on the values of the user and the enemy
                

                // This determined the index for the player input, because this will be used as added points later on.
                // Using X will give additional 1 point, Y will be 2 points and Z will be 3 points.
                int index = choices.FindIndex(a => a.Contains(rps.ReturnShape)); 
